...
export function loadConfig(): Config {
  const raw = {
    llmProvider: (process.env.LLM_PROVIDER ?? "mock") as "anthropic" | "openai" | "gemini" | "mock",
    ...
    backendSrcDir: path.resolve(ROOT_DIR, process.env.BACKEND_SRC_DIR ?? "../fidar-server/src/main/java"),
    frontendSrcDir: path.resolve(ROOT_DIR, process.env.FRONTEND_SRC_DIR ?? "../CallCenterUI/client/src"),
    frontendServerSrcDir: path.resolve(ROOT_DIR, process.env.FRONTEND_SERVER_SRC_DIR ?? "../CallCenterUI/server"),
    appBaseUrl: process.env.APP_BASE_URL ?? "http://localhost:5000",
    apiBaseUrl: process.env.API_BASE_URL ?? "http://localhost:8084/fidar/sdk/api",
    appAuthToken: process.env.APP_AUTH_TOKEN || undefined,
    appLoginUsername: process.env.APP_LOGIN_USERNAME || undefined,
    appLoginPassword: process.env.APP_LOGIN_PASSWORD || undefined,
    mcpHeadless: toBool(process.env.MCP_HEADLESS, true),
    dashboardPort: Number(process.env.DASHBOARD_PORT ?? "4700"),
    rootDir: ROOT_DIR,
    runsDir: path.join(ROOT_DIR, "runs"),
    cacheDir: path.join(ROOT_DIR, ".cache"),
    serverPort: Number(process.env.SERVER_PORT ?? "4701"),
    dbPath: path.resolve(ROOT_DIR, process.env.DB_PATH ?? "data/platform.db"),
    managedRepoDir: path.resolve(ROOT_DIR, process.env.MANAGED_REPO_DIR ?? "../generated-tests-repo"),
    managedRepoBranch: process.env.MANAGED_REPO_BRANCH ?? "main",
  };

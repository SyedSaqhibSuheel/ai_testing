...
export async function runPlaywrightTest(
  ...
  try {
    ...
    const nodeModulesPath = path.join(config.managedRepoDir, "node_modules");
    if (!existsSync(nodeModulesPath)) {
      throw new Error(
        `node_modules isn't installed in the managed repo (${config.managedRepoDir}). Run "npm install" there first.`
      );
    }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      // Use npm test which runs our custom playwright wrapper (playwright.js)
      // This works cross-platform without needing to find the .bin executable
      const isWindows = process.platform === "win32";
      const command = isWindows ? "npm.cmd" : "npm";

      const child = spawn(command, ["test"], {
        cwd: config.managedRepoDir,
        shell: true, // Required on Windows to resolve npm.cmd properly
        env: {
          ...process.env,
